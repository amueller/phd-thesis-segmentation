...
in by \citet{finley2008training}, who used to investigate the influence of
...
any mistake. The dataset is also interesting as the model of
...
The Pascal VOC has 964 training images, each having
...
obtained by majority vote over the pixel belonging to the superpixel. We
removed all superpixels which were mostly ``void'', leading to some samples
...
for the Pascal VOC dataset 21 unary potentials and 63 for the MSRC-21 dataset.
We use the same three pairwise features for both dataset: a constant feature, a color
...
%Interestingly, the $1$-slack algorithm
%catches up near the desired suboptimality and is faster in achieving the desired duality gap--even though
%the primal objective is always above SZLJSP\@. A zoom-in can be found in
%\Figref{ocr_small_duality}, right hand side.
...
on learning much longer---even though the primal of the $1$-slack algorithms is
always above the primal of the SZLJSP\@. This is caused by a looser bound
given by the dual. Remember that the stopping criterion for both cutting-plane 
...
in guaranteeing the desired suboptimality. \Figref{ocr_small_dual} illustrates this point by plotting primal and dual
objectives for the $1$-slack cutting plane and the SZLJSP algorithms.
 Looking at the training set loss in \Figref{mnist_curves} suggests that the
 high precision we demanded was not necessary, and we could have terminated the
 stochastic algorithms much earlier.
...
dataset.
%
As on MNIST, the $1$-slack algorithm stops before the SZLJSP (in terms of
iterations) because of a tighter dual.  A detailed view is shown in
\Figref{ocr_small_dual} on the right.
...
\begin{figure}
    ...
    \end{tabu}
\caption{%
    Primal objective and dual objective (dashed lines) for SZLJSP and $1$-slack cutting plane on MNIST (left) and OCR-small (right).
...
\Figref{snakes_results} shows the results on the snakes dataset.
...
The last set of experiments is on the segmentation dataset MSRC-21 and Pascal
...
First, we notice that all algorithms achieve low (approximate) duality gap, and none
...
down learning on both datasets. This is in contrast to the yeast datasets, where the converse was true,
even though yeast has a similar number of training examples to Pascal VOC.
...
We do not consider SSGD with averaging~\citep{lacoste2012simpler} here, but
found it so produce results similar to plain SSGD in tentative experiments. In
...
While the SSGD and SZLJSP algorithms have somewhat lower training error on the snakes dataset than
...
task near perfectly.
For the segmentation datasets, it is harder to judge the outcome, though there seems to be little difference
between the learning algorithms in terms of the final result.
In the next chapter, we will demonstrate how
