...
For the full pairwise multi-label model, we use branch-and-bound together with the %FIXME AD3 solver description?
...
We use this dataset as an example for very cosly inference.
...
quantified in \Figref{mnist_times}.  Clearly the $1$-slack algorithms to ``to
...
datasets are shown in \Figref{multi-label-results}, respectively. The caching
$1$-slack algorithm is much faster than the non-caching one on both datasets.
...
primal suboptimality.  On the scene dataset, the other cutting-plane base
algorithms are second fastest, while they lag behind the stochastic algorithms
on the larger yeast dataset. Again, initial convergence is faster for the
...
\begin{itemize}
    \item SZJLSP always converges faster than SSGD, with each iteration having
    ...
    \item The n-slack cutting plane algorithm was often fastest to converge in
        ...
        runtime. The caching $1$-slack algorithm converged faster in terms of
        cpu-time on almost all datasets.
\end{itemize}
...
remarkable how competative the SZLJSP algorithm is, without the need for any
such implementation tricks. This makes the SZJLSP algorithm very attractive for
the practitioner who does not want to spend much time tuning the
implementation.

We do not consider SSGD using averaging here, but found it so produce similar
results in tentative experiments. In particular, it was never faster than SZLJSP.
...
While the SSGD and SZJLSP algorithms have somewhat lower training error on the snakes than
