...
in \Eqref{main_equation}.

While $y$ could be a complicated like a parsing tree or the geometric configuration of a molecule,
many setting, such as the image segmentation setting we are interested, can be reduced to the case
where $y$ is a vector of discrete labels $\mathcal{Y} = \{0, \dotsc, k\}^n$. %TODO n varies.
...
\Eqref{mainequation} by exploiting the graph structure of the factor graph.

To obtain a linear function as in \Eqref{mainequation} from
...
structured prediction approach of \Eqref{mainequation} and
