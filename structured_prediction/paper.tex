...
\section{CRFs for Semantic Segmentation}\seclabel{semantic_segmentation}
%
...
Most conditional random fields only apply unary and pairwise potential functions. In this case, FIXME equation 4.4
becomes
...
%TODO where does that leave us now?
In Chapter~\ref{ch:exact_learning} and Chapter~\ref{ch:nyu}, we investigate
superpixel-based conditional random fields learned using structural support
vector machines. The emphasis of Chapter~\ref{ch:exact_learning} is on
understanding the learning, and how to cope with the complexity of inference in
loopy models, while Chapter~\ref{ch:nyu} extends the approach to RGB-D images
and shows how we can learn spatial relations using parametric pairwise
potentials.
