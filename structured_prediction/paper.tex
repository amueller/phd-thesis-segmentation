...
Implementation of the algorithm is straight-forward, but unfortunately
detecting convergence is often tricky.
...
datasets.  We will refer to the algorithm as STLJSP, after the authors of the
two papers, to honor the independent development of the two views of the
method.
%
The STDJSP algorithm has properties that are very simliar to
stochastic subgradient descent, but with two main advantages:
\begin{description}
\item[Stopping Criterion] Both views of STLJSP give rise to a dual objective,
...
\item[Learning Rate] While there are several theoretical results on choosing
    ...
    STLJSP removes the need for any learning rate parameter, making application
    of the algorithm in practice much simpler.
\end{description}

The STLJSP algorithm is usually used with weighted averaging, as this is known
to improve performance in similar stochastic algorithms, and empirically
improves convergence. % TODO weighted averaging here? or at SGD?
