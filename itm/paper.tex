...
\begin{figure}
\centering
\includegraphics[width=.30\textwidth]{illustration_left}
\includegraphics[width=.30\textwidth]{illustration_center}
\includegraphics[width=.30\textwidth]{illustration_right}
...
While this procedure has no strong guarantees, we found this faster
in practice than specialized algorithms for euclidean minimum spanning trees, 
which achive a better theoretical runtim of $O(n \log(n) \alpha(n))$~\citep{dtb2010}. Here
...
data is assumed to lie on a lower-dimensional manifold, embedded in a
...
In their method, for each datapoint $x$, a local estimate $\hat{d}(x)$ of the
dimensionality at $x$ is computed as
\begin{equation}
    \hat{d}(x) = \frac{\ln 2}{\ln(r_k(x) / r_{\lfloor k /2 \rfloor}(x)}.
\end{equation}
...
\end{equation}
We found this estimate to work robustly and give sensible results in all setting.
As we already used a ball-tree data structure to build the minimum spanning trees,
estimating the dimensionality resulted in neglectible computational overhead.
...
the feature dimension, but degrated otherwise (see ``faces'' and ``usps'').
Estimating the intrinsic dimensionality of the data overcomes this weakness,
and improves resuls in most cases. For all but one dataset, eigher ITM or ITM with
...
\bottomrule
\end{tabularx}
\caption{Adjusted Rand Index of $k$-Means, MeanNN, single link agglomerative clustering and
