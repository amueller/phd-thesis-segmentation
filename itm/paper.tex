...
build the minimum spanning trees, estimating the dimensionality resulted only in
...
ITM on several datasets (higher is better). The best score for
each dataset is printed in bold.\\$^\dagger$\footnotesize We were unable to make MeanNN scale to 70000 data points, as storing the
whole pairwise distance matrix seems critical.\label{results}}
\end{table}
