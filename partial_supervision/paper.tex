Most algorithms for semantic image segmentation and object-class segmentation work with strong supervision: a pixel-wise labeling
...
%TODO theoretical part reference?
% TODO repetition below?
...
In our setup, each segment might be given multiple labels. %TODO explain!
...
\end{equation}
where $Y= \{$car, bike, person$ \}$, $S_i$ enumerates all segments within an %FIXME we didn't describe the dataset yet!
...
\Tabref{muskacc}. Results were obtained using 10-fold
...
\Tabref{musk-witness} compares the witness rates of MIK-instance,
...
%parametric min cuts (CPMC,\citet{carreira2010constrained}). We use the top 200 ranked segments and discard the
%rest.
%Each segment is described using SIFT~\citet{lowe2004distinctive} and
%ColorSIFT~\citet{van2009evaluating} features, from which we compute bag of
%visual word histograms as well as histograms of oriented
%gradients~\citet{dalal2005histograms}. We construct one MI-Kernel per
...
\begin{table}
    ...
    \caption{Pixel-level accuracy on the Graz-02 dataset.
        \tablabel{tab:graz}}
\end{table}
Per-class pixel accuracies are reported in \Tabref{tab:graz}; some qualitative results are shown in \figref{results}. The overall
