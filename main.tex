...
an important open problem in computer vision. In this work, we investigates
...
\section{Summary}
This thesis explored the use of structured prediction methods for semantic
segmentation and object class segmentation of natural images, as an important
...
We focus on \emph{learning} of structural models and the interaction of
inference and learning in the neighborhood models typically employed for
semantic segmentation.
We presents an open source software implementation of a variety of
...
We demonstrate that effective use of available inference mechanisms enables
exact learning, even in the presence of loops in the underlying
factor graph, leading to state-of-the art results on the MSRC-21 dataset.
We also demonstrate the power of conditional interactions, learning spacial
interactions in a RGB-D setting. Our approach improves upon the state-of-the
...
range of datasets. This indicates that also in the task of extracting
superpixels, we can hope to achieve better results than the $k$-means based
...
is available on the internet. Using additional training data, and evaluating on
the given, manually annotated data, would be an interesting topic for future
research.
...
variables. Possible examples are consistency of larger regions, learning parts
or learning of scene classes and cooccurcences.
...
attention, while most approaches using latent variables are often limited by
...
the exception of Chapter~\ref{ch:nyu}, that explores the use of 3D features
for semantic segmentation of indoor scenes. It is clear, however, that
the input feature play an important role in the performance of any system.
...
somewhat overlooked, often being reduced to a single constant or contrast
sensitive feature.
